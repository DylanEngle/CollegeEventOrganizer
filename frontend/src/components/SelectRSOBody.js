...
{
    console.log("made it in SelectRSO");
    ...
    var user_id = ud.user_id;
        ...
        {
            method: 'post',
            url: bp.buildPath('api/selectrso'),
            headers:
            {
...
function SelectRSO()
{
    ...
    const doSelectRSO = async event =>{
      ...
      localStorage.setItem('selectedRSO', JSON.stringify(rso_id));
     ...
     return(
      <center className='HomePageBox'>
          <br></br>
          <label for="Name"><strong>Select an RSO to create an event for.</strong></label>
        <div className="card">
        ...
        <div id="registerButtonBox">
              <input type="submit" value="Create RSO" onClick={doSelectRSO} />
        </div>
      </center>
     );
};
